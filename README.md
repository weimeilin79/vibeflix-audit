...
### 🚀 Install the whole mesh: `./deploy/deploy_mesh.sh`
...
./deploy/deploy_mesh.sh
```

**Run it, don't source it.** `./deploy/deploy_mesh.sh`, never `. deploy/deploy_mesh.sh` — sourcing makes
...
`./deploy/deploy_mesh.sh --list` prints them; `--skip-gateway` stops after the app.
...
──
resume   : ./deploy/deploy_mesh.sh --from legal
```

Every phase is idempotent, so re-running is the normal way to recover:

```bash
./deploy/deploy_mesh.sh --resume        # continue after the last phase that completed
./deploy/deploy_mesh.sh --from legal    # or name one
./deploy/deploy_mesh.sh --only app      # just one
```
