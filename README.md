...
4. **Shared A2A Task Store**: Every A2A call is `POST message:send` followed by `GET /a2a/v1/tasks/{id}` polled to completion — so the *task* is the unit of state the whole mesh runs on. In cloud, Agent Runtime scales each engine to **several replicas behind a load balancer with no session affinity**, while the A2A server's default `InMemoryTaskStore` keeps tasks in a dict **private to one replica**. The `POST` therefore lands on replica *A* and the `GET` gets balanced to replica *B*, which has never heard of the task and answers `404 Task not found` — and it is not even a fair coin, because the replica that owns the task is the one busy executing it, so the balancer routes away from precisely the one you need. Measured on a real run: **1,228 misses / 1,415 polls (86.8%)**. That single fact was the hidden cause of nearly everything that looked broken elsewhere — multi-minute runs that were mostly the client losing coin flips, ~1,900 "error" spans (26% of *all* spans) that were merely 404 polls, a console chat blocked for seven minutes behind one form-designer task, and a `recovery` node re-running agents that had never failed. The fix is to keep tasks **outside** the replicas: `vertexai`'s `A2aAgent` template accepts a `task_store_builder`, so every engine now shares one store hosted by the app (`vibeflix_common/task_store.py` → `PUT/GET/DELETE /api/taskstore/{id}`), and **any replica can serve any task** (misses went to **0**, and a full audit from 5m01s → 1m44s). It lives behind the app rather than in a database because the **Agent Gateway governs HTTP egress and cannot match a gRPC channel or a raw TCP socket** — which rules out Cloud SQL/Postgres (the SDK's own `DatabaseTaskStore`) and Redis outright, and makes Firestore viable but fragile (A2A saves on *every* event, against a ~1 write/sec per-document ceiling). Plain HTTPS to a Cloud Run service is a path the engines already use — it is how they reach the MCP servers — so the auth story comes for free. Two constraints are load-bearing rather than tuning knobs: the app must run as **exactly one instance** (`--min-instances=1 --max-instances=1`, since the store is a dict in that process — and a second instance would also split the Pub/Sub mesh subscription, which is a competing-consumer queue, leaving the workflow graph half-drawn), and the endpoints are gated by a shared secret (`TASK_STORE_KEY`), because the app is deliberately public so the browser can load the console. Nothing here is durable by design: it is demo state, not a system of record, and if the app is unreachable the store degrades back to per-replica memory with a loud warning rather than failing the run.
