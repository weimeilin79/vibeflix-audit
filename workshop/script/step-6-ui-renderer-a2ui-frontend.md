...
A2UI reverses the direction. The result decides the interface: an agent looks at what the backend produced and emits a description of the UI to draw, and the frontend renders what it's handed.

The renderer emits components from a fixed schema — a card, a table, a badge, a question with options — and the frontend draws those and nothing else. The schema bounds the space of possible interfaces, and the model's job is choosing which components to use and what goes in them.
...
The model call has no output schema attached, because A2UI blocks are a text format, so reliability comes from validating and falling back. The renderer emits a panel, the app parses it, and if it fails to validate the app heals what it can and falls back to a plain rendering, so the user always gets something.
...
The obvious resolution is a second pass: deploy the engines, deploy the app, redeploy all six engines so they learn the URL. This workshop used to do that.

It's unnecessary, because the URL is computable before the app exists. Cloud Run serves every service at two addresses. One is the hashed form that `describe` reports, and the other is deterministic, built from the service name, project number and region. Both are live, and the second is derivable from what you already know.
...
Sourcing the env file matters here, because it exports the Firestore database name, which makes the MCP servers read the registries you seeded in Step 1 rather than their fallback data.
...
Try the exclusivity block too. It's the same request in North America, where a competitor holds the lock, and one branch turns red while the others pass and no contract is issued. The run carried on, the other two verdicts came back, and the vendor got a specific reason.

[DO: Ctrl+C to stop everything.]

That was the local mesh, with processes talking over localhost. The engines you deployed to Agent Runtime played no part. Step 8 runs the identical console against those.
...
It's also almost entirely ungoverned. Every agent has its own identity and IAM, but no policy sits in the traffic path, and nothing checks per call whether a particular agent may call a particular tool.

Next: identity, registry, and a deny-by-default gateway that refuses anything outside policy.
